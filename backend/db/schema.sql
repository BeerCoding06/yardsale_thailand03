...
/* ===== Buyer Wallet System (see migrations/20260429_buyer_wallet_system.sql) ===== */
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'orders' AND column_name = 'wallet_amount_used') THEN
    ALTER TABLE public.orders ADD COLUMN wallet_amount_used NUMERIC(14, 2) NOT NULL DEFAULT 0;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'orders' AND column_name = 'refund_processed_at') THEN
    ALTER TABLE public.orders ADD COLUMN refund_processed_at TIMESTAMPTZ;
  END IF;
END $orders_buyer_wallet_cols$;
