...
export const useCart = () => {
  ...
  /**
   * ดึงสต็อกล่าสุดจาก WooCommerce + ปลั๊กอิน แล้วเขียนลงตะกร้า (localStorage)
   * เรียกตอนเปิดหน้า Checkout เพื่อลดข้อความ "out of stock" จาก snapshot เก่า
   */
    ...
    try {
      ...
      const next = cart.value.map((item) => {
        ...
        const line = res.lines.find(
          (l) =>
            l.product_id === pid && Number(l.variation_id ?? 0) === Number(vid ?? 0)
        );
        if (!line) return item;
